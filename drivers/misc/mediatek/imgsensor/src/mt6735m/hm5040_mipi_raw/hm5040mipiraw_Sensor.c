...
HM5040_GAIN HM5040_D_gain_table[]=
{
  {0x0000, 100},
	{0x0008, 103},
	{0x0010, 106},
...
//    SENSORDB("[S]");
    ...
    write_cmos_sensor(0x0202,0x08);//coarse integration time
    write_cmos_sensor(0x0203,0x00);// 2048
	...
	kal_uint8 retry = 3;
		do {
			*sensor_id = ((read_cmos_sensor(0x2016) << 8) | read_cmos_sensor(0x2017));
			printk("gaochao_5040_get_imgsensor_id: 0x%x\n", *sensor_id);
...
//				*sensor_id =0xffffffff;
			 ...
			 else
		{
			*sensor_id=imgsensor_info.sensor_id;
			...
			printk("Read sensor id ok, id: 0x%x\n", imgsensor.i2c_write_id);
