...
#define LOGE(format, args...)   printk(PFX "[%s] " format, __FUNCTION__, ##args)
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("read id: 0x%x, sensor id: 0x%x\n", imgsensor_info.sensor_id,*sensor_id);

				LOG_INF("s5k3h7yxmipiraw read id success : 0x%x\n", *sensor_id);
				return ERROR_NONE;
			}
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		...
		*sensor_id = 0xFFFFFFFF;
		LOG_INF("s5k3h7yxmipiraw read id fail : 0x%x\n", *sensor_id);
