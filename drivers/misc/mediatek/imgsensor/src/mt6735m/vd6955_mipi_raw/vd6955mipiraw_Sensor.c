...
#include "vd6955mipiraw_Sensor.h"

#define PFX "VD6955R2A"
//#define LOG_WRN(format, args...) xlog_printk(ANDROID_LOG_WARN ,PFX, "[%S] " format, __FUNCTION__, ##args)
//#defineLOG_INF(format, args...) xlog_printk(ANDROID_LOG_INFO ,PFX, "[%s] " format, __FUNCTION__, ##args)
//#define LOG_DBG(format, args...) xlog_printk(ANDROID_LOG_DEBUG ,PFX, "[%S] " format, __FUNCTION__, ##args)
#define LOG_INF(format, args...)   do{} while(0)//#define LOG_INF(format, args...)	xlog_printk(ANDROID_LOG_INFO   , PFX, "[%s] " format, __FUNCTION__, ##args)

static DEFINE_SPINLOCK(imgsensor_drv_lock);


static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = VD6955MIPI_SENSOR_ID, //VD6955_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
	...
	.pre = {
		...
		.linelength  = 2750,				//record different mode's linelength
		.framelength = 2180,			//record different mode's framelength
		.startx = 0,					//record different mode's startx of grabwindow
		.starty = 0,					//record different mode's starty of grabwindow
	...
	.i2c_addr_table = {0x20,0x36, 0xff},//{0x36, 0xff},//record sensor support all write id addr, only supprt 4must end with 0xff
};
...
static struct VD6955_GAIN VD6955_gain_table[]=
{
//0x0204/0x0205
    {0x0000, 100},
    {0x0010, 106},
...
static struct VD6955_GAIN VD6955_D_gain_table[]=
{
  {0x0000, 100},
	{0x0008, 103},
	{0x0010, 106},
...
{ 2600, 1952,	  0,	0, 2600, 1952, 1296,  976,   0,	0, 1296, 976,	 0, 0, 1296, 976}, // Preview 
 ...
 { 2600, 1952,	  0,	0, 2600, 1952, 1296,  976,   0,	0, 1296, 976,	 0, 0, 1296, 976}
#else
	{ 2600, 1952,	  0,	0, 2600, 1952, 2600, 1952,   0,	0, 2592, 1944,	 0, 0, 2592, 1944}, // capture 
	{ 2600, 1952,	  0,	0, 2600, 1952, 2600, 1952,	 0,    0, 2592, 1944,	 0, 0, 2592, 1944}, // capture 
	{ 2600, 1952,	  0,	0, 2600, 1952, 2600, 1952,   0,	0, 2592, 1944,	 0, 0, 2592, 1944}, // capture 
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
    ...
    //LOG_INF("[VD6955MIPI]mycat BASEGAIN=%d\n",BASEGAIN);
    
    //gain_tmp1=(iGain % BASEGAIN);
    do {
        if (gain_tmp0 < VD6955_gain_table[i].analoggain)
            break;
        i++;
    } while (VD6955_gain_table[i].analoggain != 0);

    if (i == 0)
    {

        Reg_Cgain = 0x00;
       // LOG_INF("[brad_gain] iGain=%d gain_tmp0=%d  analoggain=%d  Reg_Cgain = 0x%x    \n",iGain,gain_tmp0,VD6955_gain_table[i].analoggain,Reg_Cgain);

    }
    else
    {
        Reg_Cgain = VD6955_gain_table[i-1].gainvalue & 0xFF;
      //  LOG_INF("[brad_gain] iGain=%d gain_tmp0=%d  analoggain=%d  Reg_Cgain = 0x%x    \n",iGain,gain_tmp0,VD6955_gain_table[i-1].analoggain,Reg_Cgain);

    }
Reg_Cgain = Reg_Cgain & 0xFF;
    //VD6955MIPI_write_cmos_sensor(0x0204,0x00);
//    VD6955MIPI_write_cmos_sensor(0x0205,Reg_Cgain);
//    VD6955MIPI_write_cmos_sensor(0x0104, 0x00); 
...
#if 1
	//gain_tmp0 = (VD6955_gain_table[i].analoggain * 100) / gain_tmp0;
	gain_tmp0 = (gain_tmp0 * 100) /  VD6955_gain_table[i-1].analoggain;
	i = 0;
	do {
        if (gain_tmp0 < VD6955_D_gain_table[i].analoggain)
            break;
		LOG_INF("[1], VD6955_D_gain_table[i].analoggain = %d", VD6955_D_gain_table[i].analoggain);
        i++;
    } while (VD6955_D_gain_table[i].analoggain != 0);
	if (i == 0)
    {
        Reg_Dgain = 0x00;
    }
    ...
    {
        Reg_Dgain = VD6955_D_gain_table[i-1].gainvalue & 0xFF;
    }
...
write_cmos_sensor(0xFBEB, 0x4C);
write_cmos_sensor(0xFBEC, 0xCC);
write_cmos_sensor(0xFBED, 0x41);
write_cmos_sensor(0xFBEE, 0x13);
write_cmos_sensor(0xFBEF, 0x4C);
write_cmos_sensor(0xFBF0, 0xB9);
write_cmos_sensor(0xFBF1, 0x41);
write_cmos_sensor(0xFBF2, 0x11);
write_cmos_sensor(0xFBF3, 0x4C);
write_cmos_sensor(0xFBF4, 0xB9);
write_cmos_sensor(0xFBF5, 0x41);
write_cmos_sensor(0xFBF6, 0x12);
write_cmos_sensor(0xFBF7, 0x4C);
write_cmos_sensor(0xFBF8, 0xBA);
write_cmos_sensor(0xFBF9, 0x41);
write_cmos_sensor(0xFBFA, 0x40);
write_cmos_sensor(0xFBFB, 0x4C);
write_cmos_sensor(0xFBFC, 0xD3);
write_cmos_sensor(0xFBFD, 0x41);
write_cmos_sensor(0xFBFE, 0x44);
...
write_cmos_sensor(0x0136, 0x0C); //2 1
...
write_cmos_sensor(0x0341,0x84);// Hex(824) = 2084
...
write_cmos_sensor(0x0100,0x00);
    ...
    write_cmos_sensor(0x0306,0x00);//pll_mult_dummy_hi
    write_cmos_sensor(0x0307,0x96);//pll_mult    
    write_cmos_sensor(0x0308,0x00);
    write_cmos_sensor(0x030a,0x00); 
    write_cmos_sensor(0x030b,0x01);
    write_cmos_sensor(0x0340,0x08); //03
    write_cmos_sensor(0x0341,0x84); //2 1
    write_cmos_sensor(0x0344,0x00); //2 1
    write_cmos_sensor(0x0345,0x04); //2 1   ;
    write_cmos_sensor(0x0346,0x00); //2 1
    write_cmos_sensor(0x0347,0x04); //2 1   ;
    write_cmos_sensor(0x0348,0x0A); //2 1
    write_cmos_sensor(0x0349,0x23); //2 1   ;
    write_cmos_sensor(0x034A,0x07); //2 1
    write_cmos_sensor(0x034B,0x9B); //2 1   ;
    write_cmos_sensor(0x034C,0x0A); //2 1
    write_cmos_sensor(0x034D,0x20); //2 1   ;x_output_size -2592
    write_cmos_sensor(0x034E,0x07); //2 1
    write_cmos_sensor(0x034F,0x98); //2 1   ;Y_output_size -1944
    write_cmos_sensor(0x040C,0x0A); //2 1
    write_cmos_sensor(0x040D,0x20); //2 1
    write_cmos_sensor(0x040E,0x07); //2 1
    write_cmos_sensor(0x040F,0x98); //2 1
    write_cmos_sensor(0x0202,0x0e); //coarse integration time ***
    write_cmos_sensor(0x0203,0x50); // 989		***
    write_cmos_sensor(0x0205,0xC0); //gain code		***
    write_cmos_sensor(0x0B04,0x01);
    write_cmos_sensor(0x0B05,0x01);
    write_cmos_sensor(0x0A02,0x1E);
    write_cmos_sensor(0x0A00,0x03);
    write_cmos_sensor(0x0A0A,0x1D);
    write_cmos_sensor(0x0A0B,0x63);
    write_cmos_sensor(0x0A0C,0x64);
    write_cmos_sensor(0x0100,0x01);
    mdelay(50);
	...
	kal_uint8 retry = 3;
		do {
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
			printk("yueqiu get_imgsensor_id: 0x%x\n", *sensor_id); //sensor ID is  0x07A3
			if (*sensor_id != imgsensor_info.sensor_id) {
				*sensor_id =0xffffffff;
				printk("read VD6955 R1A R2A bate fail\n");	  
			 ...
			 else
		{
			*sensor_id=imgsensor_info.sensor_id;
			...
			printk("yueqiu Read sensor id ok, id: 0x%x\n", imgsensor.i2c_write_id);
