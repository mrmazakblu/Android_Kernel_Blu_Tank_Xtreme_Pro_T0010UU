...
/*
	if( imgsensor.autoflicker_en ) {
		...
		if( realtime_fps >= 297 && realtime_fps <= 305 )
			...
			// Extend frame length
			write_cmos_sensor( 0xfe, 0x00 );
			write_cmos_sensor( 0x04, (imgsensor.frame_length >> 8) & 0xff );
			write_cmos_sensor( 0x03, (imgsensor.frame_length     ) & 0xff );
		}
	} else {
			// Extend frame length
			write_cmos_sensor( 0xfe, 0x00 );
...
static void sensor_init(void)
{
	...
	write_cmos_sensor( 0xfe, 0x01 );
	write_cmos_sensor( 0x6a, 0x28 );//GAIN
	write_cmos_sensor( 0x6b, 0x07 );
	write_cmos_sensor( 0x6c, 0x1c );//{0x6b,0x6c}:intgration time
	write_cmos_sensor( 0xfe, 0x00 );
