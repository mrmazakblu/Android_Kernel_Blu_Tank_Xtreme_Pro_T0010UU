...
#include "sp5407mipi_sensor.h"

#define PFX "sp5407_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = SP5407MIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
/*
	//gain = gain & 0x7ff;
	if (gain < BASEGAIN || gain > 13 * BASEGAIN) {
		LOG_INF("Error gain setting");

		if (gain < BASEGAIN)
			gain = BASEGAIN;
		else if (gain > 13 * BASEGAIN)
			gain = 13 * BASEGAIN;
	}
*/
...
write_cmos_sensor(0x3501,0x45);   //7e
...
write_cmos_sensor(0x3611,0x54);    //58   2017.3.23
...
write_cmos_sensor(0x3803,0x00);  //04
write_cmos_sensor(0x3804,0x0a);
write_cmos_sensor(0x3805,0x3f);
write_cmos_sensor(0x3806,0x07);
write_cmos_sensor(0x3807,0xaf);  //ab
write_cmos_sensor(0x3808,0x05);  //0a
write_cmos_sensor(0x3809,0x10);  //20
write_cmos_sensor(0x380a,0x03);  //07
write_cmos_sensor(0x380b,0xcc);   //98
...
write_cmos_sensor(0x3813,0x06);  //08
write_cmos_sensor(0x3814,0x03);   //01
write_cmos_sensor(0x3815,0x01);
write_cmos_sensor(0x3816,0x03);   //01
...
write_cmos_sensor(0x3820,0x8b);   //88
write_cmos_sensor(0x3821,0x01);   //00
...
write_cmos_sensor(0x4008,0x02);  //04
write_cmos_sensor(0x4009,0x09);  //13
...
#ifdef DEBUG_SENSOR_OV5695
	if(read_from_extern_flash(OV5695_OTHER_DEBUG_PATH)) {
		printk("sp_zhanghua: other debug!\n");
		OV5695_other_debug(OV5695_OTHER_DEBUG_PATH);
	}
#endif
