...
#include "hm8040mipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "HM8040_camera_sensor"
#define LOG_1 LOG_INF("HM8040,MIPI 4LANE\n")
#define LOG_2 LOG_INF("preview 3280*2464@30fps,784Mbps/lane; video 3280*2464@30fps,784Mbps/lane; capture 8M@30fps,784Mbps/lane\n")
/****************************   Modify end    *******************************************/
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = HM8040_SENSOR_ID,

    .checksum_value = 0x381e7cd3,        //checksum value for Camera Auto Test

    .pre = {
        .pclk = 140000000,            //record different mode's pclk
        .linelength = 1792,            //record different mode's linelength
        .framelength = 2534,            //record different mode's framelength
...
static imgsensor_struct imgsensor = {
    ...
    .shutter = 0x0108,                    //current shutter
    .gain = 0x00,                        //current gain
    .dummy_pixel = 0,                    //current dummypixel
    .dummy_line = 0,                    //current dummyline
    .current_fps = 275,  //full size current fps : 24fps for PIP, 30fps for Normal or ZSD
...
{{ 3280, 2464,      0,    0, 3280, 2464, 3280, 2464,      0,   0, 3280, 2464,  0,    0, 3280,  2464},  // Preview
 ...
 { 3280, 2464,    336,  434, 1952, 1088, 1952, 1088,     16,   4, 1920, 1080,  0,    0, 1920,  1080}, //hight speed video
 { 3280, 2464,     16,  254, 2592, 1448, 1296,  724,      8,   2, 1280,  720,  0,    0, 1280,  720}}; // slim video
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
    ...
    reg_gain = gain/4-16; //(((gain / BASEGAIN) - 1) << 4) + ((gain % BASEGAIN) * 16 / BASEGAIN);
    reg_gain = reg_gain & 0xFFFF;
	//A version workaround ++
	//if (reg_gain > 0x6C) //A version MAX gain is 7.75x
	//	reg_gain = 0x6C;
	//A version workaround --
...
{
   // LOG_INF("E, setting_version: 20160304 GJ mail\n");
    ...
    write_cmos_sensor(0x0309,0x01);
    write_cmos_sensor(0x030D,0x0C);
    write_cmos_sensor(0x030F,0x60);
    write_cmos_sensor(0x4287,0x01);
    #if 0
    write_cmos_sensor(0x4131,0x01);
    write_cmos_sensor(0x4132,0x20);
    write_cmos_sensor(0x4140,0x20);
    write_cmos_sensor(0x4144,0x03);
    write_cmos_sensor(0x4145,0x33);
    write_cmos_sensor(0x4146,0x31);
    write_cmos_sensor(0x4147,0x03);
    write_cmos_sensor(0x4149,0x60);
    write_cmos_sensor(0x414A,0x00);
    write_cmos_sensor(0x4261,0x00);
    write_cmos_sensor(0x4262,0x01);
    write_cmos_sensor(0x4264,0x0A);
    write_cmos_sensor(0x4012,0x00);
    write_cmos_sensor(0x4270,0x22);
    write_cmos_sensor(0x4271,0x68);
    write_cmos_sensor(0x4272,0x43);
    write_cmos_sensor(0x4273,0x44);
    write_cmos_sensor(0x4280,0x50);
    write_cmos_sensor(0x4281,0x10);
    write_cmos_sensor(0x4283,0x07);
    write_cmos_sensor(0x4284,0x00);
    write_cmos_sensor(0x4285,0x0B);
    write_cmos_sensor(0x4380,0xA6);
    write_cmos_sensor(0x4381,0x71);
    write_cmos_sensor(0x4383,0x77);
    write_cmos_sensor(0x4384,0x05);
    write_cmos_sensor(0x4385,0xA4);
    write_cmos_sensor(0x438A,0x80);
    write_cmos_sensor(0x438B,0x80);
    write_cmos_sensor(0x438C,0x80);
    write_cmos_sensor(0x4400,0x00);
    write_cmos_sensor(0x4401,0x36);
    write_cmos_sensor(0x4404,0x01);
    write_cmos_sensor(0x45FC,0x02);
    write_cmos_sensor(0x45FD,0x00);
    write_cmos_sensor(0x45FE,0x10);
    write_cmos_sensor(0x4490,0xE7);
    write_cmos_sensor(0x4491,0xE3);
    write_cmos_sensor(0x4492,0xE1);
    write_cmos_sensor(0x4493,0xE0);
    write_cmos_sensor(0x4494,0xE8);
    write_cmos_sensor(0x4495,0xF8);
    write_cmos_sensor(0x44D0,0x33);
    write_cmos_sensor(0x44D1,0x33);
    write_cmos_sensor(0x44D2,0x33);
    write_cmos_sensor(0x44D3,0x33);
    write_cmos_sensor(0x44D4,0x33);
    write_cmos_sensor(0x44D5,0x33);
    write_cmos_sensor(0x44A0,0xE7);
    write_cmos_sensor(0x44A1,0xE3);
    write_cmos_sensor(0x44A2,0xE1);
    write_cmos_sensor(0x44A3,0xE0);
    write_cmos_sensor(0x44A4,0xE8);
    write_cmos_sensor(0x44A5,0xF8);
    write_cmos_sensor(0x44B0,0xE1);
    write_cmos_sensor(0x44B1,0xE0);
    write_cmos_sensor(0x44B2,0xE8);
    write_cmos_sensor(0x44B3,0xE8);
    write_cmos_sensor(0x44B4,0xF8);
    write_cmos_sensor(0x44B5,0xF8);
    write_cmos_sensor(0x4600,0x00);
    write_cmos_sensor(0x4601,0x00);
    write_cmos_sensor(0x4602,0x00);
    write_cmos_sensor(0x4603,0x01);
    write_cmos_sensor(0x4604,0x01);
    write_cmos_sensor(0x4605,0x01);
    #else
		write_cmos_sensor(0x4131,0x01),
		write_cmos_sensor(0x4132,0x20),
		write_cmos_sensor(0x4140,0x20),
		write_cmos_sensor(0x4144,0x63),
		write_cmos_sensor(0x4145,0x33),
		write_cmos_sensor(0x4146,0x55),
		write_cmos_sensor(0x4147,0x03),
		write_cmos_sensor(0x4149,0x50),
		...
		write_cmos_sensor(0x4380,0xA6),
		write_cmos_sensor(0x4381,0x71),
		write_cmos_sensor(0x4383,0x71),
		write_cmos_sensor(0x4384,0x05),
		write_cmos_sensor(0x4385,0xA2),
		write_cmos_sensor(0x438A,0x80),
		write_cmos_sensor(0x438B,0x80),
		write_cmos_sensor(0x438C,0x80),
		write_cmos_sensor(0x4400,0x36),
		write_cmos_sensor(0x4401,0x36),
		write_cmos_sensor(0x4403,0x3F),
		write_cmos_sensor(0x4404,0x01),
		write_cmos_sensor(0x45FC,0x02),
		write_cmos_sensor(0x45FD,0x00),
		write_cmos_sensor(0x45FE,0x10),
    ...
    write_cmos_sensor(0x0350,0x73);//add 20161201
    write_cmos_sensor(0x0111,0x01);
    write_cmos_sensor(0x0112,0x0A);
    write_cmos_sensor(0x0113,0x0A);
    write_cmos_sensor(0x4B01,0x2B);
    write_cmos_sensor(0x4B20,0xDE);
    write_cmos_sensor(0x4B18,0x2C);
    write_cmos_sensor(0x4B02,0x04);
    write_cmos_sensor(0x4B43,0x06);
    write_cmos_sensor(0x4B05,0x18);
    write_cmos_sensor(0x4B0E,0x00);
    write_cmos_sensor(0x4B0F,0x0C);
    write_cmos_sensor(0x4B06,0x05);
    write_cmos_sensor(0x4B39,0x09);
    write_cmos_sensor(0x4B42,0x06);
    write_cmos_sensor(0x4B03,0x0A);
    write_cmos_sensor(0x4B04,0x06);
    write_cmos_sensor(0x4B3A,0x09);
    write_cmos_sensor(0x4B57,0x03);
    write_cmos_sensor(0x4B51,0x80);
    write_cmos_sensor(0x4B52,0xC9);
    write_cmos_sensor(0x4B11,0x0F);
    write_cmos_sensor(0x4B45,0x00);
    write_cmos_sensor(0x4B46,0x7F);
    write_cmos_sensor(0x4B47,0xC0);
    write_cmos_sensor(0x4B3B,0x13);
    write_cmos_sensor(0x4024,0x40);
    write_cmos_sensor(0x0104,0x00);
    mDELAY(10);
