...
#include "imx190mipiraw_Sensor.h"

/****************************Modify following Strings for debug****************************/
#define PFX "imx190_camera_sensor"
//#define LOG_1 LOG_INF("imx190,MIPI 2LANE\n")
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = IMX190_SENSOR_ID,
	
	.checksum_value = 0x6b3a163d,//0x9e08861f,
		...
		.startx = 0,
		.starty = 0,
		.grabwindow_width = 1376,
		.grabwindow_height = 968,
	...
	.hs_video = {
		.pclk = 234000000,              //record different mode's pclk
    	.linelength = 3232,             //record different mode's linelength
   	 	.framelength = 2400,            //record different mode's framelength
    	.startx = 0,
    	.starty = 0,
		.grabwindow_width = 1376,//,		//record different mode's width of grabwindow
		.grabwindow_height = 968,//,		//record different mode's height of grabwindow
		.mipi_data_lp2hs_settle_dc = 14,
    .max_framerate = 300,
	},
	.slim_video = {
	...
	.max_frame_length = 0x1fff,//max framelength by sensor register's limitation //lzg
  ...
  .i2c_speed = 100,//300, // i2c read/write speed
};
...
{{ 1376, 968,	  0,	0, 1376, 968, 1376,  968, 0000, 0000, 1376,  968,	  0,	0, 1376,  968}, // Preview 
 { 1376, 968,	  0,	0, 1376, 968, 1376, 968, 0000, 0000, 1376,  968,	  0,	0, 1376, 968}, // capture 
 { 1376, 968,	  0,	0, 1376, 968, 1376,  968, 0000, 0000, 1376,  968,	  0,	0, 1376,  968}, // video 
 { 1376, 968,	  0,	0, 1376, 968,  1376,  968, 0000, 0000,  1376,  968,	  0,	0, 	1376,  968}, //hight speed video 
 { 1376, 968,	  0,	0, 1376, 968, 1376,  968, 0000, 0000, 1376,  968,	  0,	0, 1376,  968}};// slim video 
...
static void set_dummy(void)
{
  ...
  write_cmos_sensor(0x0162, 0x0F);
  write_cmos_sensor(0x0163, 0x00);
  write_cmos_sensor(0x0260, (imgsensor.frame_length >>8) & 0xFF);
  write_cmos_sensor(0x0261, imgsensor.frame_length & 0xFF);	
  write_cmos_sensor(0x0262, (imgsensor.line_length >>8) & 0xFF);
  write_cmos_sensor(0x0263, imgsensor.line_length & 0xFF);
...
{
    unsigned long flags;
	kal_uint16 MaxFrameRate = imgsensor_info.max_frame_length;
	...
	{
		MaxFrameRate = 2572; //lzg
	//	LOG_INF("IMGSENSOR_MODE_VIDEO");
	}
    spin_lock(&imgsensor_drv_lock);
    ...
    if (imgsensor.frame_length > MaxFrameRate)
        imgsensor.frame_length = MaxFrameRate;
    spin_unlock(&imgsensor_drv_lock);
    shutter = (shutter < imgsensor_info.min_shutter) ? imgsensor_info.min_shutter : shutter;
    shutter = (shutter > (MaxFrameRate - imgsensor_info.margin)) ? (MaxFrameRate - imgsensor_info.margin) : shutter;
    ...
    if (imgsensor.autoflicker_en) { 
		...
		write_cmos_sensor(0x0161, imgsensor.frame_length & 0xFF);
		write_cmos_sensor(0x0260, imgsensor.frame_length >> 8);
		write_cmos_sensor(0x0261, imgsensor.frame_length & 0xFF);
		}
	} else {
		// Extend frame length
  ...
  write_cmos_sensor(0x025a, (shutter >> 8) & 0xFF);
  write_cmos_sensor(0x025b, shutter  & 0xFF);
...
static kal_uint16 set_gain(kal_uint16 gain)
{
    ...
    //	reg_gain = 0xe0;
  write_cmos_sensor_8(0x0104, 0x01);
  write_cmos_sensor(0x157, (kal_uint8)reg_gain);
  write_cmos_sensor(0x257, (kal_uint8)reg_gain);
	write_cmos_sensor_8(0x0104, 0x00); 	
	return gain;
}	/*	set_gain  */
/*
...
static void preview_setting(void)
{
    ...
    write_cmos_sensor(0x011B,0x4F);
	  ...
	  write_cmos_sensor(0x0125,0x1F);
	  write_cmos_sensor(0x0127,0x3F);
	  write_cmos_sensor(0x012A,0x18);
    write_cmos_sensor(0x012B,0x00);//
    write_cmos_sensor(0x0150,0x00);//
    write_cmos_sensor(0x0157,0x00);//
    ...
    write_cmos_sensor(0x0172,0x00);//0x03
    write_cmos_sensor(0x0189,0x00);//
    write_cmos_sensor(0x018E,0x00);//
    write_cmos_sensor(0x0257,0x00);//
    write_cmos_sensor(0x0264,0x00);
    write_cmos_sensor(0x0266,0x0A);//
    write_cmos_sensor(0x0268,0x00);//
    write_cmos_sensor(0x0270,0x01);
    write_cmos_sensor(0x0271,0x01);//
    write_cmos_sensor(0x0288,0x00);//
    write_cmos_sensor(0x0289,0x00);//
    write_cmos_sensor(0x028E,0x00);
    write_cmos_sensor(0x0301,0x05);
	  ...
	  write_cmos_sensor(0x030C,0x00);
   	write_cmos_sensor(0x030D,0x57);
	  write_cmos_sensor(0x0410,0x20);
	  write_cmos_sensor(0x0411,0x00);
	  write_cmos_sensor(0x0412,0x20);
	  write_cmos_sensor(0x0413,0x00);
	  write_cmos_sensor(0x0414,0x20);
	  write_cmos_sensor(0x0415,0x00);
	  write_cmos_sensor(0x0416,0x20);
	  write_cmos_sensor(0x0417,0x00);
	  write_cmos_sensor(0x3A1D,0x50);
	  write_cmos_sensor(0x3A1E,0x7C);
	  write_cmos_sensor(0x3A1F,0x11);
	  write_cmos_sensor(0x3B07,0x11);
	  write_cmos_sensor(0x465E,0x1B);
	  write_cmos_sensor(0x465F,0x06);    
    write_cmos_sensor(0x4A14,0x00);
    write_cmos_sensor(0x4A15,0x00);
    write_cmos_sensor(0x4A17,0x00);
    write_cmos_sensor(0x0114,0x01);
    write_cmos_sensor(0x015A,0x04);
    write_cmos_sensor(0x015A,0xBE);
    ...
    write_cmos_sensor(0x0167,0xBF);
    write_cmos_sensor(0x0169,0x00);
    write_cmos_sensor(0x016A,0x07);
    write_cmos_sensor(0x016B,0x8F);
    write_cmos_sensor(0x016C,0x05);
    write_cmos_sensor(0x016D,0x60);
    write_cmos_sensor(0x016E,0x03);
    write_cmos_sensor(0x016F,0xC8);
    write_cmos_sensor(0x0174,0x01);
    write_cmos_sensor(0x0175,0x01);
    write_cmos_sensor(0x0176,0x01);
    write_cmos_sensor(0x0188,0x00);
    write_cmos_sensor(0x018A,0x00);
    write_cmos_sensor(0x018B,0x98);
    write_cmos_sensor(0x018C,0x0A);
    write_cmos_sensor(0x018D,0x0a);
    write_cmos_sensor(0x025A,0x05);
    write_cmos_sensor(0x025B,0xAC);
    write_cmos_sensor(0x0260,0x07);
    write_cmos_sensor(0x0261,0xDE);
    write_cmos_sensor(0x0262,0x13);
    write_cmos_sensor(0x0263,0x68);
    write_cmos_sensor(0x0264,0x00);
    write_cmos_sensor(0x0265,0x50);
    write_cmos_sensor(0x0266,0x0A);
    write_cmos_sensor(0x0267,0x6F);
    write_cmos_sensor(0x0268,0x00);
    write_cmos_sensor(0x0269,0x00);
    write_cmos_sensor(0x026A,0x07);
    write_cmos_sensor(0x026B,0x8F);
    write_cmos_sensor(0x026C,0x0A);
    write_cmos_sensor(0x026D,0x20);
    write_cmos_sensor(0x026E,0x07);
    write_cmos_sensor(0x026F,0x90);
    write_cmos_sensor(0x0274,0x00);
    write_cmos_sensor(0x0275,0x00);
    write_cmos_sensor(0x0288,0x00);
    write_cmos_sensor(0x028A,0x01);
    write_cmos_sensor(0x028B,0x15);
    write_cmos_sensor(0x028C,0x0A);
    write_cmos_sensor(0x028D,0x0A);
    write_cmos_sensor(0x0624,0x05);
    write_cmos_sensor(0x0625,0x60);
    write_cmos_sensor(0x0626,0x03);
    write_cmos_sensor(0x0627,0xC8);
    write_cmos_sensor(0x5B26,0x00);
    write_cmos_sensor(0x016C,0x05);
    write_cmos_sensor(0x016D,0x60);
    write_cmos_sensor(0x026C,0x05);
    write_cmos_sensor(0x026D,0x60);
    write_cmos_sensor(0x016E,0x03);
    write_cmos_sensor(0x016F,0xC8);
    write_cmos_sensor(0x026E,0x03);
    write_cmos_sensor(0x026E,0xC8);
    write_cmos_sensor(0x016C,0x05);
    write_cmos_sensor(0x016D,0x60);
    write_cmos_sensor(0x026C,0x05);
    write_cmos_sensor(0x026D,0x60);
    ...
    write_cmos_sensor(0x016F,0xC8);
    write_cmos_sensor(0x026E,0x03);
    write_cmos_sensor(0x026F,0xC8);
    write_cmos_sensor(0x0600,0x00);
    write_cmos_sensor(0x0601,0x00);
    write_cmos_sensor(0x015A,0x0b);
    write_cmos_sensor(0x015B,0x20);
    write_cmos_sensor(0x018A,0x0b);
    write_cmos_sensor(0x018B,0x20);
    write_cmos_sensor(0x0160,0x09);
    write_cmos_sensor(0x0161,0x60);
    write_cmos_sensor(0x0100,0x01);
   	printk("Exit preview_setting\n");
