...
#define __s5k3l8pddl_OTP_SUPPORT__
#define DEF_S5K3L8_OTP
#endif
/*===FEATURE SWITH===*/
...
#ifdef DEF_S5K3L8_OTP
#define OTP_SIZE 1868
//#define LSC_ADDR	0x3B
#define LSC_ADDR	0x1001
unsigned char otp_data[OTP_SIZE] ;
extern int s5k3l8_read_eeprom(unsigned int addr, unsigned char *data, unsigned int size);
#endif
...
#if defined(__s5k3l8pddl_OTP_SUPPORT__)

struct s5k3l8pddl_wb_otp_struct {
    int flag;
    int MID;
    int Year;
    int Month;
...
#define RGr_ratio_Typical 0x237 //0x22c
#define BGr_ratio_Typical 0x292 //0x236
#define GbGr_ratio_Typical 0x513 //0x400

//static int g_s5k3l8pddl_wb_otp_init;
...
#if defined(__s5k3l8pddl_OTP_SUPPORT__)
static kal_uint16 read_eeprom_byte(kal_uint16 addr)
{
    kal_uint16 get_byte=0;
    char pu_send_cmd[2] = {(char)(addr >> 8) , (char)(addr & 0xFF) };
...
void s5k3l8pddl_wb_read_OTP(struct s5k3l8pddl_wb_otp_struct *otp)
{
   	kal_uint16 RGr_ratio, BGr_ratio, GbGr_ratio;


  	RGr_ratio = (read_eeprom_byte(0xc2b)<<8) | read_eeprom_byte(0xc2c);
  	BGr_ratio = (read_eeprom_byte(0xc2d)<<8) | read_eeprom_byte(0xc2e);
  	GbGr_ratio = (read_eeprom_byte(0xc2f)<<8) | read_eeprom_byte(0xc30);
/*
  	RGr_ratio = (read_eeprom_byte(0x1d)<<8) | read_eeprom_byte(0x1e);
  	BGr_ratio = (read_eeprom_byte(0x1f)<<8) | read_eeprom_byte(0x20);
  	GbGr_ratio = (read_eeprom_byte(0x21)<<8) | read_eeprom_byte(0x22);
*/
...
void s5k3l8pddl_wb_apply_OTP(struct s5k3l8pddl_wb_otp_struct *otp)
{
    int R_gain,B_gain,Gb_gain,Gr_gain,Base_gain;

    LOG_INF("s5k3l8pddl_wb_apply_OTP start.\n");


    R_gain = (RGr_ratio_Typical*1000) / otp->RGr_ratio;
    B_gain = (BGr_ratio_Typical*1000) / otp->BGr_ratio;
    Gb_gain = (GbGr_ratio_Typical*1000) / otp->GbGr_ratio;
    Gr_gain = 1000;
    Base_gain = R_gain;
    ...
    LOG_INF("s5k3l8pddl_wb_apply_OTP complete.\n");
}
...
#ifdef  DEF_S5K3L8_OTP
				s5k3l8_read_eeprom(LSC_ADDR, otp_data, OTP_SIZE);
				s5k3l8_read_eeprom(1, test, 1);
#endif
                return ERROR_NONE;
            }
...
{
#if defined(__s5k3l8pddl_OTP_SUPPORT__)
    struct s5k3l8pddl_wb_otp_struct otp;
#endif
    ...
    spin_unlock(&imgsensor_drv_lock);
				...
				s5k3l8pddl_wb_read_OTP(&otp);
				s5k3l8pddl_wb_apply_OTP(&otp);
