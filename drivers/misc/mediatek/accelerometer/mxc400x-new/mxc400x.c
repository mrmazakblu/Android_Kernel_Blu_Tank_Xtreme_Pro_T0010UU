...
#define GSENSOR_HWINFO
#ifdef GSENSOR_HWINFO
...
//static s16 cali_sensor_xdata;
//static s16 cali_sensor_ydata;
static s16 cali_sensor_data;



static const struct i2c_device_id mxc400x_i2c_id[] = { { "mxc400x", 0 }, { }, };
...
static int mxc400x_i2c_detect(struct i2c_client *client, struct i2c_board_info *info);
...
static int MXC400X_CheckDeviceID(struct i2c_client *client)
{
	...
	if((databuf[0] != MXC400X_ID)  && (databuf[0] != MXC400X_ID_BAK))
	{
...
//	struct mxc400x_i2c_data *obj = i2c_get_clientdata(client);
	...
	{
		printk("memsic Sensor power status is newest!\n");
		return MXC400X_SUCCESS;
	}
	if(enable == 1)
	{
	...
	if(res <= 0)
	{
		printk("memsic set power mode failed!\n");
		return MXC400X_ERR_I2C;
	}
	sensor_power = enable;
	mdelay(50);//lyon 300
...
static int MXC400X_ReadSensorDataFactory(struct i2c_client *client, char *buf, int bufsize)
{
	...
	int res = 0;
//	static int pre_data[MXC400X_AXES_NUM] = {0,0,9805};
//	static int now_data[MXC400X_AXES_NUM] = {0,0,9805};
//	static u8 init_status = 0;


	memset(databuf, 0, sizeof(u8)*10);
	printk("ATAmemsic entry %s\n",__func__);

	if(NULL == buf)
	{
        printk("mxc4005 buf is null !!!\n");
		return -1;
	}
	if(NULL == client)
	{
		*buf = 0;
         printk("mxc4005 client is null !!!\n");
		return MXC400X_ERR_STATUS;
	}

	if (atomic_read(&obj->suspend) )
	{
		printk("mxc4005 sensor in suspend read not data!\n");
		return MXC400X_ERR_GETGSENSORDATA;
	}

	if((res = MXC400X_ReadData(client, obj->data)))
	{
		printk("mxc4005 I2C error: ret value=%d", res);
		return MXC400X_ERR_I2C;
	}
	else
	{
		...
		sprintf(buf, "%04x %04x %04x", cali_sensor_xdata, cali_sensor_ydata,cali_sensor_data);
        //	printk("ATAmemsic acc raw x =%04x, y =%04x, z =%04x \n",cali_sensor_xdata,cali_sensor_ydata,cali_sensor_data);
...
static int MXC400X_ReadRawData(struct i2c_client *client, char *buf)
{
	...
	{
		//sprintf(buf, "MXC400X_ReadRawData %04d %04d %04d", obj->data[MXC400X_AXIS_X], obj->data[MXC400X_AXIS_Y], obj->data[MXC400X_AXIS_Z]);
...
static long mxc400x_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	struct mxc400x_i2c_data *obj = (struct mxc400x_i2c_data*)i2c_get_clientdata(client);
	char strbuf[MXC400X_BUFSIZE];
	char strfacbuf[MXC400X_BUFSIZE];
	int databuf[MXC400X_BUFSIZE];
	void __user *data;
	...
	switch (cmd) {
		...
		case GSENSOR_IOCTL_READ_SENSORDATA:
			...
			MXC400X_ReadSensorData(client, databuf, MXC400X_BUFSIZE);
			//MXC400X_ReadSensorDataFactory(client, strfacbuf, MXC400X_BUFSIZE);
			sprintf(strfacbuf, "%x %x %x",databuf[0],databuf[1],cali_sensor_data);
			if(copy_to_user(data, strfacbuf, strlen(strfacbuf)+1))
			{
				err = -EFAULT;
				break;
			}
...
static int mxc400x_enable_nodata(int en)
{
	...
	GSE_LOG("ctsmemsic MXC400X_enable_nodata OK en = %d sensor_power = %d\n", en, sensor_power);
	enable_status = en;
	return 0;
}
...
static int mxc400x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("memsic call %s++++++++++\n", __func__);
	 ...
	 {
			printk("[mxc400x_local_init]: %s, line %d\n", __func__, __LINE__);
...
static int __init mxc400x_driver_init(void)
{
    ...
    printk("memsic hw->i2c_num = %d hw->direction = %d\n", hw->i2c_num, hw->direction);
