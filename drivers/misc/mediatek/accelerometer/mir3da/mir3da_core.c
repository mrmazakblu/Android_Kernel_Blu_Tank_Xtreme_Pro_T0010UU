...
#if MIR3DA_TEMP_CALIBRATE
    {
		if(mir3da_temp_calibrate(&x,&y,&z)==0)
			bLoad = FILE_EXIST;
		goto EXIT;
    }
#endif
...
int mir3da_temp_calibrate(int *x,int *y, int *z)
{
	...
	else if((abs((abs(*x)-MIR3DA_OFFSET_SEN))<MIR3DA_OFFSET_MAX)&&(abs(*y)<MIR3DA_OFFSET_MAX))
	{
	...
	else if((abs((abs(*y)-MIR3DA_OFFSET_SEN))<MIR3DA_OFFSET_MAX)&&(abs(*x)<MIR3DA_OFFSET_MAX))
	{
