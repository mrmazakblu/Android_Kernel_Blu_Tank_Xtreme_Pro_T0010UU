...
#define ACCELEROMETER_HWINFO
#ifdef ACCELEROMETER_HWINFO
...
#define MI_ERR(format, ...)             if(1){printk(KERN_ERR MI_TAG format "\n", ## __VA_ARGS__);}
...
static char OffsetFileName[] = "/data/misc/miraGSensor/miraGSensorOffset.txt";
static char OffsetFolerName[] = "/data/misc/miraGSensor/";
...
#ifdef CONFIG_COMPAT
static long mir3da_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long err = 0;
...
{
	struct mir3da_i2c_data *obj = obj_i2c_data;
...
/*----------------------------------------------------------------------------*/
static DRIVER_ATTR(enable,          S_IWUSR | S_IRUGO,  mir3da_enable_show,             mir3da_enable_store);
...
#endif
#if FILTER_AVERAGE_ENHANCE
static DRIVER_ATTR(average_enhance, S_IWUSR | S_IRUGO,  mir3da_average_enhance_show,    mir3da_average_enhance_store);
#endif
...
static int mir3da_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if(NULL == mir_handle){
        MI_ERR("chip init failed !\n");
        result = -EINVAL;//avoid reboot while no gsensor
        goto exit_init_failed;
    }
...
static const struct of_device_id accel_of_match[] = {
	#if 0	// kcm modify by phf, 20160614, gsensor 兼容
...
static int __init mir3da_init(void)
{
	...
	const char *name = "mediatek,mir3da"; //"mediatek,da226";	/* kcm modify by phf, 20160514 */
