...
#define AF_DRVNAME "GC5024AF"
#define I2C_SLAVE_ADDRESS        0x18
#define I2C_REGISTER_ID            0x18
#define PLATFORM_DRIVER_NAME "lens_actuator_gc5024af"
#define AF_DRIVER_CLASS_NAME "actuatordrv_gc5024af"
...
#ifdef LensdrvCM3
inline static int getAFMETA(__user stGC5024AF_MotorMETAInfo * pstMotorMETAInfo)
{
    stGC5024AF_MotorMETAInfo stMotorMETAInfo;
    ...
    if(copy_to_user(pstMotorMETAInfo , &stMotorMETAInfo , sizeof(stGC5024AF_MotorMETAInfo)))
    {
        LOG_INF("copy to user failed when getting motor information \n");
    }
...
#if defined(AF_EN)			//leimin
	if(mt_set_gpio_mode(GPIO_CAMERA_AF_EN_PIN,GPIO_MODE_00)){LOG_INF("[AF->EN] set gpio mode failed!! \n");}
	if(mt_set_gpio_dir(GPIO_CAMERA_AF_EN_PIN,GPIO_DIR_OUT)){LOG_INF("[AF->EN] set gpio dir failed!! \n");}
	if(mt_set_gpio_out(GPIO_CAMERA_AF_EN_PIN,GPIO_OUT_ONE)){LOG_INF("[AF->EN] set gpio failed!! \n");}
#endif
...
// Q1 : Try release multiple times.
 int GC5024AF_Release(struct inode * a_pstInode, struct file * a_pstFile)
{
    LOG_INF("Start \n");
...
#if defined(AF_EN)
	if(mt_set_gpio_mode(GPIO_CAMERA_AF_EN_PIN,GPIO_MODE_00)){LOG_INF("[AF->DISABLE] set gpio mode failed!! \n");}
	if(mt_set_gpio_dir(GPIO_CAMERA_AF_EN_PIN,GPIO_DIR_OUT)){LOG_INF("[AF->DISABLE] set gpio dir failed!! \n");}
	if(mt_set_gpio_out(GPIO_CAMERA_AF_EN_PIN,GPIO_OUT_ZERO)){LOG_INF("[AF->DISABLE] set gpio failed!! \n");}

    LOG_INF("AF_EN Release - End\n");
#endif
	  LOG_INF("End \n");
    return 0;
}
...
void GC5024AF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
...
static int __init GC5024AF_i2C_init(void)
{
...
static void __exit GC5024AF_i2C_exit(void)
{
    platform_driver_unregister(&g_stAF_Driver);
}

module_init(GC5024AF_i2C_init);
module_exit(GC5024AF_i2C_exit);
#endif
//MODULE_DESCRIPTION("GC5024AF lens module driver");
