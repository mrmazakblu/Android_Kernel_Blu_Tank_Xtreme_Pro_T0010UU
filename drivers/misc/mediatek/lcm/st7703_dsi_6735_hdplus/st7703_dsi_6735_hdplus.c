...
#if defined(DROI_PRO_PU6T)
#include "st7703_dsi_6735_hdplus_pu6t.h"
#endif
...
static unsigned int lcm_compare_id(void)
{
	...
	#if defined(BUILD_LK)
		printf("st7703_hdplus_dsi %s id = 0x%04x\n", __func__, id);
	#else
		printk("st7703_hdplus_dsi %s id = 0x%04x\n", __func__, id);
	#endif
	return (0x0D == id)?1:0;
...
// ---------------------------------------------------------------------------
LCM_DRIVER st7703_hdplus_dsi_vdo_drv =
{
	.name			= "st7703_dsi_6735_hdplus",
