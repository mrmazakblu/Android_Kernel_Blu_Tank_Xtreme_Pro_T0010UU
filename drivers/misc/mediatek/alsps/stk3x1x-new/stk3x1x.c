...
#if defined(TYD_PRO_F6)||defined(DROI_PRO_F5C_LEAGOO4)||defined(DROI_PRO_PF5_LEAGOO4)
	#define STK_LT_N_CT	26
	#define STK_HT_N_CT	76
#else
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
		...
		APS_LOG("jax8,fae ct tracking\n");
			...
			if(ret == 0)
			{
				...
				if(word_data > 0)
				{
					...
					APS_LOG("%s: jax11,ct_value        = %d\n",__func__, ct_value);
					APS_LOG("%s: ax11,word_data        = %d\n",__func__, word_data);
					APS_LOG("%s: jax11,flag            = 0x%x\n",__func__, flag);
					APS_LOG("%s: jax11,ps_distance_last= %d\n",__func__, obj->ps_distance_last);
					if((word_data < ct_value) && ((ct_value - word_data) > 2) && ((flag&STK_FLG_PSINT_MASK)==0) && (obj->ps_thd_update == false))
					{
						APS_LOG("jax12,update psi\n");
	...
	{
		obj->psi_set = obj->psi;
		APS_LOG("jax10,psi_set has value\n");
...
#endif
	APS_LOG("jax6,ps_report_interrupt_data\n");
	if(ps_report_interrupt_data(near_far_state))
	{
						APS_ERR("call ps_report_interrupt_data fail\n");
	}
#ifdef CONFIG_DROI_GESTURE
#ifdef REPORT_TRIGGER
		droi_gestrue_trigger(near_far_state ?GES_NON_EFFECT:GES_RIGHT);
#endif
#endif
...
static int stk3x1x_enable_ps(struct i2c_client *client, int enable, int validate_reg)
{
	...
	if((enable + counter) == 1){
		ret = _stk3x1x_enable_ps(client,enable,validate_reg);
	}
	enable > 0 ? counter ++ :(counter > 0 ? counter --: counter);

	return ret;
}

static int dg_enable(int en)
{
	return stk3x1x_enable_ps(stk3x1x_obj->client,en,1);
}

#ifndef REPORT_TRIGGER
enum{
	DROI_ENTRY,
	DROI_HOLD,
	DROI_LEAVE
};
static int dg_getdata(void)
{
	int value,st;
	static int state = DROI_LEAVE;

	ps_get_data(&value,&st);
		...
		case 0:
			switch(state){
				...
				case DROI_ENTRY:
					state = DROI_HOLD;
					break;
				//case DROI_HOLD:break;
			}
			break;
	}

	value = GES_NON_EFFECT;
	if(state == DROI_ENTRY)
		value = GES_RIGHT;

	return value;
}
static struct sensor_ops so={
	.enable=dg_enable,
	.getdata=dg_getdata,
};
#error asdf
...
#endif
#else
static inline int stk3x1x_enable_ps(struct i2c_client *client, int enable, int validate_reg)
{
	return _stk3x1x_enable_ps(client,enable,validate_reg);
}
#endif
/*----------------------------------------------------------------------------*/
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
