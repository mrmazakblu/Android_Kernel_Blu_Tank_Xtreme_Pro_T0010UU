...
static char tyd_resolution[30]={"already"};
...
static void tyd_add_lcm_hw_info(const char* found_lcm)
{
			//char* lcm_width;
			//char* lcm_height;
			//char temp_width[8] = {0};
			//char temp_height[8] = {0};
			//lcm_width = (char*)temp_width;
			//lcm_height = (char*)temp_height;
			//sprintf(lcm_width, "%d", DISP_GetScreenWidth());
			//sprintf(lcm_height, "%d", DISP_GetScreenHeight());
			//sprintf(tyd_resolution," %s * %s ",lcm_width,lcm_height);
			hw_info.name = (found_lcm == NULL)?"null":found_lcm;
			hw_info.more = tyd_resolution;
...
disp_lcm_handle *disp_lcm_probe(char *plcm_name, LCM_INTERFACE_ID lcm_id, int is_lcm_inited)
{
	...
	if (_lcm_count() == 0) {
		...
		#ifdef LCM_HW_INFO
    			tyd_add_lcm_hw_info(lcm_drv->name);
		#endif
		lcmindex = 0;
	} else {
		if (plcm_name == NULL) {
