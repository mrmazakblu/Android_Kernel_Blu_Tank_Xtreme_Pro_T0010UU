...
int led_rgb_gpio_init(struct platform_device *pdev)
{
	...
	ledrgbctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(ledrgbctrl)) {
		dev_err(&pdev->dev, "Cannot find flashlight pinctrl!");
		ret = PTR_ERR(ledrgbctrl);
	}
        //led red green blue initialization
	red_light_h = pinctrl_lookup_state(ledrgbctrl, "led_rgb_red_1");
	if (IS_ERR(red_light_h)) {
		ret = PTR_ERR(red_light_h);
		PK_DBG("%s : pinctrl err, red_light_h\n", __func__);
	}

	red_light_l = pinctrl_lookup_state(ledrgbctrl, "led_rgb_red_0");
	if (IS_ERR(red_light_l)) {
		ret = PTR_ERR(red_light_l);
		PK_DBG("%s : pinctrl err, red_light_l\n", __func__);
	}

	green_light_h = pinctrl_lookup_state(ledrgbctrl, "led_rgb_green_1");
	if (IS_ERR(green_light_h)) {
		ret = PTR_ERR(green_light_h);
		PK_DBG("%s : pinctrl err, green_light_h\n", __func__);
	}

	green_light_l = pinctrl_lookup_state(ledrgbctrl, "led_rgb_green_0");
	if (IS_ERR(green_light_l)) {
		ret = PTR_ERR(green_light_l);
		PK_DBG("%s : pinctrl err, green_light_l\n", __func__);
	}

	blue_light_h = pinctrl_lookup_state(ledrgbctrl, "led_rgb_blue_1");
	if (IS_ERR(blue_light_h)) {
		ret = PTR_ERR(blue_light_h);
		PK_DBG("%s : pinctrl err, blue_light_h\n", __func__);
	}

	blue_light_l = pinctrl_lookup_state(ledrgbctrl, "led_rgb_blue_0");
	if (IS_ERR(blue_light_l)) {
		ret = PTR_ERR(blue_light_l);
		PK_DBG("%s : pinctrl err, blue_light_l\n", __func__);
	}
...
static int led_rgb_gpio_set(unsigned long Led_Rgb_Type, unsigned long Val)
{
	...
	case LED_RGB_RED:
		if (Val == 1)
			pinctrl_select_state(ledrgbctrl, red_light_h);
		else
			pinctrl_select_state(ledrgbctrl, red_light_l);
		break;
	case LED_RGB_GREEN:
		if (Val == 1)
			pinctrl_select_state(ledrgbctrl, green_light_h);
		else
			pinctrl_select_state(ledrgbctrl, green_light_l);
		break;
	case LED_RGB_BLUE:
		if (Val == 1)
			pinctrl_select_state(ledrgbctrl, blue_light_h);
		else
			pinctrl_select_state(ledrgbctrl, blue_light_l);
		break;
	default:
