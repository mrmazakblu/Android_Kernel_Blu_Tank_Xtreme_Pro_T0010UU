...
s32 gesture_event_handler(struct input_dev * dev)
{
    ...
    /** package: -head 4B + track points + extra info-
        ...
        */
	ret = gt1x_i2c_read(GTP_REG_WAKEUP_GESTURE, doze_buf, 8);
	if (ret < 0) {
        return 0;
    }

    printk("0x%x = 0x%02X,0x%02X,0x%02X,0x%02X,0x%02X", GTP_REG_WAKEUP_GESTURE,
        doze_buf[0], doze_buf[1], doze_buf[2], doze_buf[3], doze_buf[4]);
    ...
    GTP_DEBUG("0x%x = 0x%02X,0x%02X,0x%02X,0x%02X,0x%02X", GTP_REG_WAKEUP_GESTURE,
        doze_buf[0], doze_buf[1], doze_buf[2], doze_buf[3], doze_buf[4]);
	...
	if (ges_type == 0 || !QUERYBIT(gestures_flag, ges_type)) {
		...
		if ((doze_buf[0] == 'a') || (doze_buf[0] == 'b') || (doze_buf[0] == 'c') ||
			...
			(doze_buf[0] == 'w') || (doze_buf[0] == 'y') || (doze_buf[0] == 'z') || (doze_buf[0] == '>') || (doze_buf[0] == '^'))
	 	{
			if ('o' == doze_buf[0])
			{
				printk("Gesture[%c] wake up.\n", doze_buf[0]);
			...
			{
				printk("Gesture[%c] wake up.\n", doze_buf[0]);
				//strcpy(gesture_tmp,"d");
			//agold_tpd_get_gesture(gesture_tmp);
				input_report_key(dev, KEY_D, 1);
				input_sync(dev);
				input_report_key(dev, KEY_D, 0);
				input_sync(dev);
		...
		{
				printk("Gesture[%dul] double click wake up.\n", doze_buf[0]);
				//strcpy(gesture_tmp,"doubletap");
			//agold_tpd_get_gesture(gesture_tmp);
#if defined (DROI_PRO_F5C_YH)
			    if ((0x01 == doze_buf[4]) || (0x02 == doze_buf[4]) || (0x04 == doze_buf[4]))
				{
					doze_buf[0] = 0x00;
    				gt1x_i2c_write(GTP_REG_WAKEUP_GESTURE, doze_buf, 1);
    				gesture_enter_doze();
		...
		{
				printk("Gesture[%dul] right wake up.\n", doze_buf[0]);
		...
		{
				printk("Gesture[%dul] left wake up.\n", doze_buf[0]);
		...
		{
				printk("Gesture[%dul] down wake up.\n", doze_buf[0]);
