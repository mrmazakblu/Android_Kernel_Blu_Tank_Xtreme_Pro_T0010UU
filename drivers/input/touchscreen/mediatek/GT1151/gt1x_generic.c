...
s32 gt1x_init_panel(void)
{
    ...
    ret = gt1x_i2c_read_dbl_check(GTP_REG_CONFIG_DATA, opr_buf, 1);
    if (ret < 0) {
        return ret;
    }
    drv_cfg_version = cfgs[sensor_id][0];
    flash_cfg_version = opr_buf[0];
    /* compare drv vension and flash version to decide if update cfg */
    if((drv_cfg_version < 0x7F) && (drv_cfg_version > flash_cfg_version))
    {
        GTP_INFO("drv_cfg_version=%d , flash_cfg_version=%d\n", drv_cfg_version, flash_cfg_version);
...
s32 gt1x_read_version(struct gt1x_version_info * ver_info)
{
	...
	sprintf(tpd_chip_name, "GT%s", product_id);
	return 0;
}
...
#endif
#if defined(DROI_PRO_F6_G2_MINI)
	if (touch_num) {
#else
	else if (touch_num) {
#endif
...
s32 gt1x_init(void)
{
    ...
    //sprintf(gt1x_fw_version,"v 0x%02x", temp_data[0]);
