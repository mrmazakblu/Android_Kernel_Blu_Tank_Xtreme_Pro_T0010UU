...
#if 1//kyler add smartwakeup 20160617
static bool	gestrue_en;
int tyd_gesture_en = 0;
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	input_set_capability(tpd->dev, EV_KEY, KEY_GESTURE_TOP_V);
	input_set_capability(tpd->dev, EV_KEY, KEY_GESTURE_RIGHT_V);
	...
	__set_bit(KEY_GESTURE_TOP_V, tpd->dev->keybit);
	__set_bit(KEY_GESTURE_RIGHT_V, tpd->dev->keybit);
#endif
    ...
    /* add hw info vendor id */
	gt1x_i2c_read(GTP_REG_VERSION, temp_data, 12);
	hw_info.id=temp_data[10]&0x0F; 

    /* add hw info CFG version */
...
static int tpd_event_handler(void *unused)
{
	...
	s32 ret = 0;
	u8 point_data[11] = { 0 };
	struct sched_param param = {.sched_priority = (4 | 0x0) };
...
{
	if (RECOVERY_BOOT == get_boot_mode())	// recovery mode not need for tpd initialize
		return 0;
#ifdef TPD_POWER_SOURCE_CUSTOM
#ifdef GTP_CONFIG_OF
