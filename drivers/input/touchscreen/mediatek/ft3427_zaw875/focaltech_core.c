...
static char tpd_chip_name[10] = "FT";
...
int glove_on(void)
{
	...
	int ret = 0;
	u8 status = 0;
    ret = fts_write_reg(i2c_client, 0xC0, on);
	if(ret < 0)
	{
		printk("glove_on i2c write error\n");
		return -ret;
	}
    ret = fts_read_reg(i2c_client, 0xC0, &status);
	if(ret < 0)
	{
		printk("glove_on i2c read error\n");
	}
	printk("glove_on status is %d\n", status);
	return 0;
}
int glove_off(void)
{
	...
	int ret = 0;
	u8 status = 0;
    ret = fts_write_reg(i2c_client, 0xC0, off);
	if(ret < 0)
	{
		printk("glove_off i2c write error\n");
		return -1;
	}
    ret = fts_read_reg(i2c_client, 0xC0, &status);
	if(ret < 0)
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		if (tpd_touchinfo(&cinfo, &pinfo)) {
...
#if defined(TYD_PRO_B38_PUB)
			if (tpd_dts_data.use_tpd_button) {
				if ((cinfo.y[0] <= 1300 && cinfo.y[0] != 0) && (pinfo.y[0] > 1300)
				&& ((pinfo.p[0] == 0) || (pinfo.p[0] == 2))) {
					printk("Dummy key release --->\n");
					tpd_button(pinfo.x[0], pinfo.y[0], 0);
					input_sync(tpd->dev);
					continue;
				}

			if ((cinfo.y[0] > 1300) || (pinfo.y[0] > 1300)) {
				if (finfo.y[0] > 1300) {
			...
			if (tpd_dts_data.use_tpd_button) {
#if defined(DROI_PRO_F6_YX)
				if ((cinfo.y[0] <= (tpd_dts_data.tpd_resolution[1] + 1) && cinfo.y[0] != 0)
				&& (pinfo.y[0] > (tpd_dts_data.tpd_resolution[1] + 1))
				&& ((pinfo.p[0] == 0) || (pinfo.p[0] == 2))) {
#else
				...
				&& ((pinfo.p[0] == 0) || (pinfo.p[0] == 2))) {
                    ...
                    #if defined(DROI_PRO_FQ5C)||defined(DROI_PRO_FQ5CW)
        	            input_report_key(tpd->dev, BTN_TOUCH, 0);
        	            input_mt_sync(tpd->dev);
                    #else
						tpd_button(pinfo.x[0], pinfo.y[0], 0);
...
#if defined(DROI_PRO_F6_YX)
			if ((cinfo.y[0] > (tpd_dts_data.tpd_resolution[1] + 1))
			|| (pinfo.y[0] > (tpd_dts_data.tpd_resolution[1] + 1))) {
				if (finfo.y[0] > (tpd_dts_data.tpd_resolution[1] + 1)) {
#else
			if ((cinfo.y[0] > TPD_RES_Y) || (pinfo.y[0] > TPD_RES_Y)) {
				if (finfo.y[0] > TPD_RES_Y) {
					...
					if ((cinfo.p[0] == 0) || (cinfo.p[0] == 2)) {
							TPD_DEBUG("Key press --->\n");
                            #if defined(DROI_PRO_FQ5C)||defined(DROI_PRO_FQ5CW)
                            if((pinfo.x[0]>=60)&&(pinfo.x[0]<=180)&&(pinfo.y[0]>=1340)&&(pinfo.y[0]<=1420)){
                            			input_report_key(tpd->dev, BTN_TOUCH, 1);
                            			input_report_abs(tpd->dev, ABS_MT_TOUCH_MAJOR, 1);
                            			input_report_abs(tpd->dev, ABS_MT_TRACKING_ID,pinfo.id[0]);
                            			input_report_abs(tpd->dev, ABS_MT_POSITION_X, 120);
                            			input_report_abs(tpd->dev, ABS_MT_POSITION_Y, 1380);
                            }else if((pinfo.x[0]>=300)&&(pinfo.x[0]<=420)&&(pinfo.y[0]>=1340)&&(pinfo.y[0]<=1420)){
                            			input_report_key(tpd->dev, BTN_TOUCH, 1);
                            			input_report_abs(tpd->dev, ABS_MT_TOUCH_MAJOR, 1);
                            			input_report_abs(tpd->dev, ABS_MT_TRACKING_ID,pinfo.id[0]);
                            			input_report_abs(tpd->dev, ABS_MT_POSITION_X, 360);
                            			input_report_abs(tpd->dev, ABS_MT_POSITION_Y, 1380);
                            }else if((pinfo.x[0]>=540)&&(pinfo.x[0]<=660)&&(pinfo.y[0]>=1340)&&(pinfo.y[0]<=1420)){
                            			input_report_key(tpd->dev, BTN_TOUCH, 1);
                            			input_report_abs(tpd->dev, ABS_MT_TOUCH_MAJOR, 1);
                            			input_report_abs(tpd->dev, ABS_MT_TRACKING_ID,pinfo.id[0]);
                            			input_report_abs(tpd->dev, ABS_MT_POSITION_X, 600);
                            			input_report_abs(tpd->dev, ABS_MT_POSITION_Y, 1380);
                            };
                            #else
                                    tpd_button(pinfo.x[0], pinfo.y[0], 1);
...
static ssize_t glove_mode_store(struct class *class, struct class_attribute *attr,const char *buf, size_t count)
{
	...
	if(value)
		...
		glove_en = false;
	printk(KERN_ERR"dingxueqi test glove_en%d\n",glove_en);
	return count;
}
...
static struct class_attribute cls_attr[]={
    ...
    __ATTR(gloenable, 0664, glove_mode_show, glove_mode_store),
};
...
//i2c_smbus_read_i2c_block_data(i2c_client, 0xA6, 1, &data);	//read firmware version
      fts_read_reg(i2c_client, 0xa6, &data);
	sprintf(fw_version,"v 0x%02x", data);
	//i2c_smbus_read_i2c_block_data(i2c_client, 0xA3, 1, &data);	//read TP ID
      fts_read_reg(i2c_client, 0xa3, &data);
       if(data == 0x0A)
	{
		hw_info.id=data;
		sprintf(tpd_chip_name,"%s", "FT5x16");
	}
	else if(data == 0x06)
	{
		hw_info.id=data;
		sprintf(tpd_chip_name,"%s", "FT6x06");
	}
	else if(data == 0x55)
	{
		hw_info.id=data;
	...
	{
		hw_info.id=data;
		sprintf(tpd_chip_name,"%s", "FT5436i");
	}
	else if(data == 0x14)
	{
		hw_info.id=data;
		sprintf(tpd_chip_name,"%s", "FT5336");
	}
	else if(data == 0x36)
	{
		hw_info.id=data;
		sprintf(tpd_chip_name,"%s", "FT6x36");
	}
	else if(data == 0x54)
	{
		hw_info.id=data;
		sprintf(tpd_chip_name,"%s", "FT3427");
	}
	else if(data == 0x64)
	{
		hw_info.id=data;
		sprintf(tpd_chip_name,"%s", "FT6336");
	}
	else
	{
		hw_info.id=data;
		sprintf(tpd_chip_name,"%s", "FT****");
	}
...
static void tpd_resume(struct device *h)
{
	...
	msleep(400);
	wake_unlock(&gesture_chrg_lock);
	printk(KERN_ERR"dingxueqi  func=%s,LINE=%d,gestrue_en=%d\n",__func__,__LINE__,gestrue_en);
	if(1)
	{
