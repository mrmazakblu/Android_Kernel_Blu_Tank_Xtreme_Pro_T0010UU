...
int fts_Gesture_init(struct input_dev *input_dev)
{
	...
	input_set_capability(input_dev, EV_KEY, KEY_GESTURE_TOP_V);
	input_set_capability(input_dev, EV_KEY, KEY_GESTURE_RIGHT_V);
	...
	__set_bit(KEY_GESTURE_TOP_V, input_dev->keybit);
	__set_bit(KEY_GESTURE_RIGHT_V, input_dev->keybit);
	return 0;
}
...
static void fts_check_gesture(struct input_dev *input_dev,int gesture_id)
{
	...
	switch(gesture_id)
	{
		...
		case GESTURE_RIGHT_V:
				input_report_key(tpd->dev, KEY_GESTURE_RIGHT_V, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, KEY_GESTURE_RIGHT_V, 0);
				input_sync(tpd->dev);
			break;
		case GESTURE_TOP_V:
				input_report_key(tpd->dev, KEY_GESTURE_TOP_V, 1);
				input_sync(tpd->dev);
				input_report_key(tpd->dev, KEY_GESTURE_TOP_V, 0);
				input_sync(tpd->dev);
			break;
	        default:
