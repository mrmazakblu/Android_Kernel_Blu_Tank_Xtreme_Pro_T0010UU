...
static char tpd_chip_name[10] = "GT";
...
GTP_INFO("songqi enter tpd_power_on\n");
reset_proc:
	tpd_gpio_output(GTP_RST_PORT, 0);
	tpd_gpio_output(GTP_INT_PORT, 0);
    msleep(10);
    ...
    mt_set_gpio_out(GPIO_CTP_EN_PIN, GPIO_OUT_ONE);
       ...
       // hwPowerOn(MT6325_POWER_LDO_VGP1, VOL_2800, "TP");
		 ret1=regulator_set_voltage(tpd->reg, 2800000, 2800000);  // set 2.8v
	if (ret1)
		TPD_DMESG("regulator_set_voltage() failed!\n");
	ret1=regulator_enable(tpd->reg);  //enable regulator
	if (ret1)
...
#if 1/*defined(TYD_PRO_X1_GB)*/
    if (opr_buf[0] == 0x0)
    {
        GTP_ERROR("Failed to get chip-type, gtp_chip_type=0 ");
        gtp_chip_type = CHIP_TYPE_GT9;
	    tpd_load_status = 0;
        return;
    }
...
{
//GTP_INFO("songqi enter tpd_i2c_probe\n");
    s32 err = 0;
    s32 ret = 0,retval = 0;
...
//#ifdef VELOCITY_CUSTOM
    ...
    {
        GTP_INFO("mtk_tpd: tpd_misc_device register failed\n");
    }
...
#endif
GTP_INFO("songqi next gtp_read_version\n");
...
//cuizhaojun    Read version
#if defined(DROI_PRO_FQ5C_XWP)||defined(DROI_PRO_FQ5CW_XWP)
    i2c_read_bytes(client,GTP_REG_CONFIG_DATA,temp_data,GTP_CONFIG_MAX_LENGTH);
    sprintf(fw_version,"V%d", temp_data[0]);
#else
    i2c_read_bytes(client,GTP_REG_CONFIG_DATA,temp_data,GTP_CONFIG_MAX_LENGTH);
    sprintf(fw_version,"v 0x%02x", temp_data[0]);
#endif

GTP_INFO("songqi next gtp_init_panel\n");
...
GTP_INFO("songqi next gt91xx_config_proc\n");
    ...
    init_wr_node(client);
#endif
GTP_INFO("songqi next kthread_run\n");
	 ...
	 */
GTP_INFO("songqi next tpd_gpio_as_int\n");
	tpd_gpio_as_int(GTP_INT_PORT);
 ...
 */
GTP_INFO("songqi next of_find_matching_node\n");
...
#endif
GTP_INFO("songqi tpd_load_status = %d\n",tpd_load_status);
    tpd_load_status = 1;
GTP_INFO("songqi tpd_load_status = %d\n",tpd_load_status);
    return 0;
}
        ...
        if (DOZE_ENABLED == doze_status)
        {
            ...
            if (ret > 0)
            {
                ...
                else if (doze_buf[2] == '^')
                {
                    GTP_INFO("Wakeup by gesture(%c), light up the screen!", doze_buf[2]);
                    doze_status = DOZE_WAKEUP;
                    input_report_key(tpd->dev, KEY_F, 1);
                    input_sync(tpd->dev);
                    input_report_key(tpd->dev, KEY_F, 0);
                    input_sync(tpd->dev);
                    doze_buf[2] = 0x00;
                    gtp_i2c_write(i2c_client_point, doze_buf, 3);
                }
...
#elif defined(TPD_HAVE_BUTTON)//tyd songqi
	key_value = point_data[3 + 8 * touch_num];
	if (key_value || pre_key)
        {
		...
		printk("tpd id %d)(%d, %d)[%d]", id, input_x, input_y, input_w);
		if(key_value)
                	tpd_down(input_x, input_y, input_w, 1);
		else
			tpd_up(0,0,0);
            touch_num = 0;
            pre_touch = 0;
...
{
GTP_INFO("songqi enter tpd_local_init\n");
#if GTP_ESD_PROTECT
	...
	if (IS_ERR(tpd->reg)) {
		 GTP_INFO("regulator_get() failed!\n");
	}
GTP_INFO("songqi next i2c_add_driver(&tpd_i2c_driver)\n");
...
static s8 gtp_enter_sleep(struct i2c_client *client)
{
	...
	*/
    ...
    #ifdef TPD_POWER_SOURCE_CUSTOM
         // hwPowerDown(MT6325_POWER_LDO_VGP1, "TP");
		ret1=regulator_disable(tpd->reg);

		if (ret1!=0)
		{
		    GTP_INFO("hwPowerOn err:ret1:%d ",ret1);
		}
    #else
        hwPowerDown(MT65XX_POWER_LDO_VGP2, "TP");
    #endif
