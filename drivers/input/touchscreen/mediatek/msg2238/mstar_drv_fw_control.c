...
#if defined(DROI_PRO_F6_ZR_M82)
#include "msg22xx_xxxx_update_bin_f6_zr.h" // for MSG22xx
#include "msg22xx_yyyy_update_bin_f6_zr.h"
#else
...
#include "msg2xxx_jf2_xxxx_update_bin.h" // for MSG28xx
#include "msg2xxx_jf2_yyyy_update_bin.h"
#elif defined(DROI_PRO_PU6T_JF)
#include "msg2836a_xxxx_update_bin.h" // for MSG2836A
#include "msg2836a_yyyy_update_bin.h"
#else
...
#else
#if defined(DROI_PRO_FQ5C_ZG)                     // support at most 16 types of gesture wakeup mode
...
#if defined(CONFIG_ENABLE_CHIP_TYPE_MSG28XX) && defined(CONFIG_UPDATE_FIRMWARE_BY_SW_ID)
#include <linux/hw_module_info.h>
extern int fw_update_flag;
static char fw_version[20] = "v";
static char tpd_chip_name[10] = "MSG2XXX";
...
//                    input_report_key(g_InputDevice, RESERVER2, 1);
                    input_report_key(g_InputDevice, KEY_D, 1);
                    input_sync(g_InputDevice);
//                    input_report_key(g_InputDevice, RESERVER2, 0);
                    input_report_key(g_InputDevice, KEY_D, 0);
                    input_sync(g_InputDevice);
...
static void _DrvFwCtrlMsg28xxGetTpVendorCode(u8 *pTpVendorCode)
{
    ...
    if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG2836A)
    {
        u16 nReadAddr = 0;
        u8  szTmpData[4] = {0};
...
static s32 _DrvFwCtrlUpdateFirmwareCash(u8 szFwData[][1024], EmemType_e eEmemType)
{
    ...
    else if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG2836A) // (CHIP_TYPE_MSG28XX:0x85) (CHIP_TYPE_MSG2836A:0xbf)
    {
...
static s32 _DrvFwCtrlUpdateFirmwareBySdCard(const char *pFilePath)
{
    ...
    else if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG2836A)
    {
    ...
    if ((eSwId == eVendorId) || (IS_FORCE_TO_UPDATE_FIRMWARE_ENABLED))
    {
        ...
        else if ((g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG2836A) && fsize == 133120/* 130KB */)
        {
...
{
    u16 readMajor = 0, readMinor = 0;
    u8 *readpVersion = NULL;
    u32 nCrcMainA, nCrcMainB;
    u32 i;
    u16 nUpdateBinMajor = 0, nUpdateBinMinor = 0;
    u16 nMajor = 0, nMinor = 0;
    ...
    if (nCrcMainA == nCrcMainB)
    {
        ...
        eSwId = eMainSwId;
#ifdef DROI_PRO_F5C_JF2
            if (eSwId == MSG28XX_JF2_SW_ID_XXXX)
#elif defined DROI_PRO_PU6T_JF
            if (eSwId == MSG2836A_SW_ID_XXXX)
#else
        ...
        if (nUpdateBinMinor > nMinor)
        {
        	...
        	{
				hw_info.id=CHIP_TYPE_MSG28XX;
				sprintf(tpd_chip_name,"%s", "MSG28XX");
					DrvFwCtrlGetCustomerFirmwareVersion(&readMajor, &readMinor, &readpVersion);
				sprintf(fw_version, "%03d%03d", nUpdateBinMajor, nUpdateBinMinor);
			    fw_update_flag = 1;
				hw_module_info_add(&hw_info); //add hw info
			}
			if(g_ChipType==CHIP_TYPE_MSG2836A)
			{
				hw_info.id=CHIP_TYPE_MSG2836A;
				sprintf(tpd_chip_name,"%s", "MSG2836A");
					DrvFwCtrlGetCustomerFirmwareVersion(&readMajor, &readMinor, &readpVersion);
				sprintf(fw_version, "%03d%03d", nUpdateBinMajor, nUpdateBinMinor);
			    fw_update_flag = 1;
				hw_module_info_add(&hw_info); //add hw info
...
static void _DrvFwCtrlUpdateFirmwareBySwIdDoWork(struct work_struct *pWork)
{
    ...
    if (nRetVal == 0)
    {
        ...
        if (g_ChipType == CHIP_TYPE_MSG22XX || g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG2836A)
        {
...
u8 DrvFwCtrlGetChipType(void)
{
    ...
    if (nChipType != CHIP_TYPE_MSG21XX &&   // (0x01)
		...
		nChipType != CHIP_TYPE_MSG2836A)	// (0xBF)
    {
        nChipType = 0;
    }
...
s32 DrvFwCtrlUpdateFirmwareBySdCard(const char *pFilePath)
{
    ...
    if (g_ChipType == CHIP_TYPE_MSG21XXA || g_ChipType == CHIP_TYPE_MSG22XX || g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG2836A)
    {
...
u16 DrvFwCtrlChangeFirmwareMode(u16 nMode)
{
    ...
    else if (g_ChipType == CHIP_TYPE_MSG21XXA || g_ChipType == CHIP_TYPE_MSG22XX || g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG2836A)
    {
...
s32 DrvFwCtrlEnableProximity(void)
{
    ...
    else if (g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG2836A)
    {
...
//    DBG(&g_I2cClient->dev, "*** %s() *** mutex_lock(&g_Mutex)\n", __func__);  // add for debug
    ...
    if (g_GestureDebugMode == 1 && g_GestureWakeupFlag == 1)
    {
        ...
        if (g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG2836A)
        {
            nReportPacketLength = GESTURE_DEBUG_MODE_PACKET_LENGTH;
            pPacket = _gGestureWakeupPacket;
        }
