...
//    const char *vcc_i2c_name = "vcc_i2c";
   ...
   // g_I2cClient->addr=0x26;
	...
	if((g_ChipType==CHIP_TYPE_MSG28XX) && (fw_update_flag==0))
	{
		hw_info.id=CHIP_TYPE_MSG28XX;
		sprintf(tpd_chip_name,"%s", "MSG5846");
        	DrvFwCtrlGetCustomerFirmwareVersion(&readMajor, &readMinor, &readpVersion);
    		sprintf(fw_version, "%03d%03d", readMajor, readMinor);

		hw_module_info_add(&hw_info); //add hw info
	}
	if((g_ChipType==CHIP_TYPE_MSG2836A) && (fw_update_flag==0))
	{
	...
	{
		hw_info.id=CHIP_TYPE_MSG22XX;
		sprintf(tpd_chip_name,"%s", "MSG2238");
        	DrvFwCtrlGetCustomerFirmwareVersion(&readMajor, &readMinor, &readpVersion);
    		sprintf(fw_version, "%03d%03d", readMajor, readMinor);
