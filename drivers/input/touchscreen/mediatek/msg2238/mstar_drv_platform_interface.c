...
#if defined(CONFIG_ENABLE_GESTURE_WAKEUP)    //add 20160906 for tyd
static ssize_t gesture_mode_show(struct class *class, struct class_attribute *attr, char *buf)
{
    return sprintf(buf, "%s\n", ((g_GestureWakeupMode[0]&0xFFFF) == 0xFFFF) ? "enable" : "disable");
}
...
static ssize_t gesture_mode_store(struct class *class, struct class_attribute *attr,const char *buf, size_t count)
{
	...
	if(value)
		g_GestureWakeupMode[0]  = 0xFFFF;
	else
		g_GestureWakeupMode[0]  = 0;

	printk(KERN_ERR"test g_GestureWakeupMode[0] %d\n",g_GestureWakeupMode[0]);
...
s32 /*__devinit*/ MsDrvInterfaceTouchDeviceProbe(struct i2c_client *pClient, const struct i2c_device_id *pDeviceId)
{
	...
	if(cls){
		nRetVal2=class_create_file(cls,cls_attr);
		if(nRetVal2)
