...
*/
    ...
    input_set_capability(g_InputDevice, EV_KEY, KEY_D);
    ...
    {
        input_mt_init_slots(g_InputDevice, MUTUAL_MAX_TOUCH_NUM, INPUT_MT_DIRECT | INPUT_MT_DROP_UNUSED); // for MSG26xxM/MSG28xx/MSG2836A
    }
#endif //CONFIG_ENABLE_TYPE_B_PROTOCOL
...
void DrvPlatformLyrEsdCheck(struct work_struct *pWork)
{
    ...
    while (i < 3)
    {
        ...
        if (g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG2836A)
        {
            rc = IicReadData(SLAVE_I2C_ID_DWI2C, &szData[0], MUTUAL_DEMO_MODE_PACKET_LENGTH); // for MSG26xxM/MSG28xx/MSG2836A
