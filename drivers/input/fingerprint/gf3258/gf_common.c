...
/* debug log setting */
u8 g_debug_level = DEBUG_LOG; //ERR_LOG;
...
/******tyd  add tmd hardware info start*****/
...
#if 0
	gf_dev->pins_miso_spi = pinctrl_lookup_state(gf_dev->pinctrl_gpios, "state_spi_miso_init");
	...
	*/
	...
	case GF_IOC_INPUT_KEY_EVENT:
		...
		if (GF_KEY_HOME == gf_key.key) {
			key_input = 68;//GF_KEY_INPUT_HOME;
...
static int gf_probe(struct spi_device *spi)
{
	...
	gf_dev->gBuffer = kzalloc(10, GFP_KERNEL);
	if (!gf_dev->gBuffer) {
		status = -ENOMEM;
		goto err1;
	}
	...
	__set_bit(GF_KEY_INPUT_F10, gf_dev->input->keybit);//ADD  F10
...
#ifdef READ_CHIP_ID //add for test SPI communication in kernel level start
